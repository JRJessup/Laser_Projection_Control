...
double setpoint = 1080;//I am setting it to move through 100 degrees
double Kp = 0.84104;// you can set these constants however you like depending on trial & error
double Ki = 5.1389;
double Kd = 0.016108;
...
void loop(){
  ...
  // }
  // else {
  //   digitalWrite(dir1, LOW);//stop motion
  //   digitalWrite(dir2, LOW);
  // }
   
   if (angle > (setpoint - 5) && angle < (setpoint + 5)) {
...
void Achange() //these functions are for finding the encoder counts
{
  ...
  switch (state)
  {
    case 1:
    {
      if (lastState == 2) count++;
      if (lastState == 4) count--;
      break;
    }
    case 2:
    {
      if (lastState == 1) count--;
      if (lastState == 3) count++;
      break;
    }
    case 3:
    {
